...
public class PathGenerateV3 : MonoBehaviour
{
    ...
    void UpdatePositionAndRotation(GameObject prefab)
    {
        if (prefab == S1)
        {
            currentPosition += currentRotation * new Vector3(0, 0, 10); // แนวตั้ง
        }
        else if (prefab == S2)
        {
            currentPosition += currentRotation * new Vector3(10, 0, 0); // แนวนอน
        }
        else if (prefab == L1)
        {
            currentRotation *= Quaternion.Euler(0, -90, 0); // เลี้ยวซ้ายแนวตั้ง
            currentPosition += currentRotation * new Vector3(0, 0, 10);
        }
        else if (prefab == L2)
        {
            currentRotation *= Quaternion.Euler(0, -90, 0); // เลี้ยวซ้ายแนวนอน
            currentPosition += currentRotation * new Vector3(10, 0, 0);
        }
        else if (prefab == R1)
        {
            currentRotation *= Quaternion.Euler(0, 90, 0); // เลี้ยวขวาแนวตั้ง
            currentPosition += currentRotation * new Vector3(0, 0, 10);
        }
        else if (prefab == R2)
        {
            currentRotation *= Quaternion.Euler(0, 90, 0); // เลี้ยวขวาแนวนอน
            currentPosition += currentRotation * new Vector3(10, 0, 0);
        }
    }
}